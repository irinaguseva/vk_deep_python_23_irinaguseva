...
class TestPredictMessageMood(unittest.TestCase):
    ...
    def test_predict_message_mood_bad(self):
        ...
        result = predict_message_mood("Тестируем получение оценки неуд", self.model)
        self.assertEqual(result, 'неуд')


    def test_predict_message_mood_good(self):
        self.model.predict = MagicMock(return_value=0.9)
        result = predict_message_mood("Тестируем получение оценки отл", self.model)
        self.assertEqual(result, 'отл')


    def test_predict_message_mood_normal(self):
        self.model.predict = MagicMock(return_value=0.5)
        result = predict_message_mood("Тестируем получение оценки норм", self.model)
        self.assertEqual(result, 'норм')


    def test_predict_message_mood_raises_error_when_missing_arguments(self):
        with self.assertRaises(TypeError):
            predict_message_mood()

        with self.assertRaises(TypeError):
            predict_message_mood(model=self.model)

        with self.assertRaises(TypeError):
            predict_message_mood(message="Hello")


    def test_predict_message_mood_with_another_thresholds(self):
        self.model.predict = MagicMock(return_value=1.1)
        result = predict_message_mood(message="This is a normal message", model=self.model,
    ...
    def test_predict_message_mood_giving_prediction_of_wrong_type(self):
        self.model.predict = MagicMock(return_value='МодельПетрович')
        with self.assertRaises(TypeError):
            predict_message_mood(
                "Тестируем передачу str вместо корректного результата работы модели", 
                self.model)

        self.model.predict = MagicMock(return_value={})
        with self.assertRaises(TypeError):
            predict_message_mood(
                "Тестируем передачу dict() вместо корректного результата работы модели", 
                self.model)

        self.model.predict = MagicMock(return_value=[])
        with self.assertRaises(TypeError):
            predict_message_mood(
                "Тестируем передачу list() вместо корректного результата работы модели", 
                self.model)
